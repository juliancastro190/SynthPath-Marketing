...
5. **Set environment variables**: service → **Variables** tab → add:
   ...
   ```
   You do **not** need `ELEVENLABS_API_KEY`, `RESEND_API_KEY`, or the
   `YOUTUBE_*` publisher variables for the heartbeat's own scheduled tasks
   — anything that would use them (`youtube_produce`, `send_email`,
   `publish_youtube_video`) always requires your explicit yes, which you
   *can* now give from Discord if you want that to actually work when you
   approve it live; add them if so, skip them if you're fine with the
   heartbeat only ever declining those unattended (it always will
   regardless — this only affects whether a *live Discord* approval of
   one of those tools can actually complete). Note `send_email`
   specifically needs Resend, not SMTP — plain SMTP doesn't work from
   Railway at all (outbound SMTP is blocked at the platform level; see
   `griffin/tools/send.py` for how this was discovered). `YOUTUBE_
   REFRESH_TOKEN` has to come from running `youtube_auth.py` locally
   first (it needs a browser) — see `.env.example`'s Tier 11 section —
   then paste the result in here as a Railway variable.
...
7. Railway should already be reading `nixpacks.toml` from this repo to
   install `requirements-discord.txt` into a venv and run
   `discord_main.py` — see the comment in that file for why it's built
   this specific way (two earlier, simpler attempts each failed
   differently on a live deploy). If the logs show it running `main.py`
   ...
   merged) — that's what caused it the first time here.
...
Change `nixpacks.toml`'s two `requirements-discord.txt`/`discord_main.py`
references to `requirements-heartbeat.txt`/`heartbeat_main.py`.
